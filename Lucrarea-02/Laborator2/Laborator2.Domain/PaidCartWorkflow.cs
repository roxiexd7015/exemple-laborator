...
namespace Laborator2.Domain
{
    ...
    {
        public async Task<IPaidCartEvent> ExecuteAsync(PayCartCommand command, Func<ProductCode, TryAsync<bool>> checkProductExists, Func<ProductCode, ProductQuantity, TryAsync<bool>> checkStock, Func<Client, TryAsync<bool>> checkAddress)
        {
            UnvalidatedCartState unvalidatedCart = new UnvalidatedCartState(command.InputCart);
            ICartStates cart = await ValidateProductsCart(checkProductExists, checkStock, checkAddress, unvalidatedCart);
